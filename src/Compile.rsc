...
str init2js(start[Form] form) 
  = "var $state = {
    '  <for (/Question q := form, q has prompt) {>
    '  <q.name>: <defaultValue(q.\type)>,
    '  <}>
    '}";


str defaultValue((Type)`integer`) = "0";
str defaultValue((Type)`boolean`) = "false";
str defaultValue((Type)`string`) = "\'\'";
...
HTMLElement question2html(q:(Question)`<Str l> <Id x>: <Type t>`)
  = div([label([text("<l>"[1..-1])]), type2widget(q, t, false)], id=divId(q));

HTMLElement question2html(q:(Question)`<Str l> <Id x>: <Type t> = <Expr _>`)
  = div([label([text("<l>"[1..-1])]), type2widget(q, t, true)], id=divId(q));
...
str expr2js((Expr)`<Expr x> / <Expr y>`) 
  = "(function () { const div = <expr2js(y)>; return div !== 0 ? Math.floor(<expr2js(x)> / div) : 0; })()";
